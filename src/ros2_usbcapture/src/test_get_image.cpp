...
namespace wmj
{
    Image_get_test::Image_get_test(const rclcpp::NodeOptions &options) : rclcpp::Node("listener", options)
    {
        ...
        // 创建订阅者
        sub_image = this->create_subscription<base_interfaces::msg::MatWithTime>("camera_image", 10, std::bind(&Image_get_test::call_back, this, std::placeholders::_1));
    }

    void Image_get_test::call_back(const base_interfaces::msg::MatWithTime::SharedPtr msg)
    {
        ...
        std::cout << "Elemsize = " << image.elemSize() << std::endl; // 一个像素点的大小  CV_8U3C=1*3
        cv::waitKey(1);
