...
namespace wmj
{
    ...
    Binocular::Binocular(double f)
    {
        ...
        m_new_camera_matrix_left = cv::getOptimalNewCameraMatrix(
            m_camera_matrix_left, m_dist_coeffs_left, cv::Size(1280, 1024), 0,
            cv::Size(1280, 1024));
        m_new_camera_matrix_right = cv::getOptimalNewCameraMatrix(
            m_camera_matrix_right, m_dist_coeffs_right, cv::Size(1280, 1024), 0,
            cv::Size(1280, 1024));
    }

    cv::Point2f Binocular::pixelToImagePlane(cv::Point2f &input_point,
    ...
    cv::Point2f Binocular::distortionCorrection(cv::Point2f &input_point,
                                                bool direction)
    {
        cv::Mat RR;
        cv::Mat PP;
        ...
        cv::undistortPoints(input_point_vec, undistort_point_vec, camera_matrix,
                            dist_coeffs, RR, new_camera_matrix);
        output_point = pixelToImagePlane(undistort_point_vec[0], new_camera_matrix);
        return output_point;
    }

    cv::Point3f Binocular::getPosition(cv::Point2f point_avg_left,
                                       cv::Point2f point_avg_right,
                                       bool direction)
    {
        cv::Point3f position;
    ...
    std::pair<cv::Point3f, double> Binocular::getPositionAngle(
        std::pair<cv::Point2f, cv::Point2f> point_avg_left,
        std::pair<cv::Point2f, cv::Point2f> point_avg_right)
    {
        cv::Point3f position_left[2], position_right[2], position;
        double angle;
        //以左目为基准
        position_left[0] =
            this->getPosition(point_avg_left.first, point_avg_right.first, 0);
        position_right[0] =
            this->getPosition(point_avg_left.second, point_avg_right.second, 0);
        //以右目为基准
        // position_left[1] = this->getPosition(point_avg_left.first,
        // point_avg_right.first,1); position_right[1] =
        // this->getPosition(point_avg_left.second, point_avg_right.second,1);
        position = (position_left[0] + position_right[0]) / 2;
        angle = atan2(position_right[0].x - position_left[0].x,
                      position_left[0].y - position_right[0].y);

        return std::make_pair(position, angle);
    }
    std::pair<cv::Point3f, cv::Point3d>
    Binocular::getPositionAngle(std::vector<cv::Point2f> points_left,
                                std::vector<cv::Point2f> points_right)
    {
        ...
        position = (position_left[0] + position_left[1] + position_left[2] +
                    position_left[3]) /
                   4;
    ...
    Monocular::Monocular() { setParam(); }
    void Monocular::setParam()
    {
        cv::FileStorage fs(BINO_CFG, cv::FileStorage::READ);
        fs["M1"] >> m_CameraMat_Left;
        fs["D1"] >> m_DistMat_Left;
        fs["M2"] >> m_CameraMat_Right;
        fs["D2"] >> m_DistMat_Right;
        fs.release();
        cv::FileStorage mono(MONO_CFG, cv::FileStorage::READ);
        mono["cameraMatrix"] >> m_CameraMat;
        mono["distCoeffs"] >> m_DistMat;
        mono.release();
        cv::FileStorage armor(ARMORS_CFG,
                              cv::FileStorage::READ);

        armor["ArmorDetector"]["armor_small_height"] >> m_small_height;
        armor["ArmorDetector"]["armor_small_width"] >> m_small_width;
        armor["ArmorDetector"]["armor_large_height"] >> m_large_height;
        armor["ArmorDetector"]["armor_large_width"] >> m_large_width;
        armor.release();
    }

    std::pair<cv::Point3f, double>
    Monocular::calcArmorDeepth(std::vector<cv::Point2f> vertices,
                               wmj::ARMORTYPE type, int choose)
    {
        std::vector<cv::Point3f> op_v;
        if (type == wmj::ARMOR_SMALL)
            GetObjPoints(m_small_width, m_small_height, op_v);
        else if (type == wmj::ARMOR_LARGE)
            GetObjPoints(m_large_width, m_large_height, op_v);
        else
            return std::make_pair(cv::Point3f(500.f, 500.f, 500.f), 0);

        cv::Mat opoints(op_v, CV_32FC1);
        ...
        if (choose == 0)
        {
            ret = solvePnP(opoints, cv::Mat(ipoints_v), m_CameraMat_Left,
                           m_DistMat_Left, rvec, tvec);
        }
        else
        {
            ret = solvePnP(opoints, cv::Mat(ipoints_v), m_CameraMat_Right,
                           m_DistMat_Right, rvec, tvec);
        }

        if (ret)
        {
            return std::make_pair(cv::Point3f(tvec.ptr<double>(0)[2] * 1000,
                                              tvec.ptr<double>(0)[0] * -1000,
                                              tvec.ptr<double>(0)[1] * (-1) * 1000),
                                  angle);
        }
        else
        {
    ...
    std::pair<cv::Point3f, double>
    Monocular::getPositionAngle(std::vector<cv::Point2f> vertices,
                                wmj::ARMORTYPE type)
    {
        std::vector<cv::Point3f> op_v;
        if (type == wmj::ARMOR_SMALL)
        ...
        cv::Mat rvec, tvec;
        bool ret;
        ret = solvePnP(opoints, cv::Mat(ipoints_v), m_CameraMat, m_DistMat, rvec, tvec, false, cv::SOLVEPNP_AP3P);
        cv::Rodrigues(rvec, m_Rvec);
        double Z = asin(-1 * m_Rvec.at<double>(1, 2));
        double angle = asin(m_Rvec.at<double>(1, 3) / cos(Z));//步兵yaw欧拉角
        if (ret)
        {
            return std::make_pair(cv::Point3f(tvec.ptr<double>(0)[2] * 1000,
                                              tvec.ptr<double>(0)[0] * -1000,
    ...
    void Monocular::GetObjPoints(float _width, float _height,
                                 std::vector<cv::Point3f> &op_v)
    {
        op_v.clear();
